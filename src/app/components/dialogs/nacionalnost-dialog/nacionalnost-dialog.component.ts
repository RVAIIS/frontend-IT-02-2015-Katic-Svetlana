...
export class NacionalnostDialogComponent implements OnInit {
  ...
  public add(): void {
    ...
    this.snackBar.open("Uspešno dodata nacionalnost: " + this.data.naziv, "U redu", { duration: 2500 });
  }

  public update(): void {
    this.nacionalnostService.updateNacionalnost(this.data);
    this.snackBar.open("Uspešno modifikovana nacionalnost: " + this.data.id, "U redu", { duration: 2500 });
  }

  public delete(): void {
    this.nacionalnostService.deleteNacionalnost(this.data.id);
    this.snackBar.open("Uspešno obrisana nacionalnost: " + this.data.id, "U redu", { duration: 2500 });
  }
